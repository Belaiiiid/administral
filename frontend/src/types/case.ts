...
export interface CaseSummary {
  ...
  waitingDays: number;
  /**
   * The three fields below are present only on rows from the CAF instructor
   * list (`GET /agent/cases/list`) — absent on the plain instruction-queue
   * rows from `GET /agent/cases`. Each is read exactly as an earlier pipeline
   * stage wrote it; `undefined`/`null` means that stage has not run, never a
   * value recomputed in the browser.
   */
  /** 0–100 completeness rate. */
  completionRate?: number | null;
  /** Outcome of the coherence check. */
  coherenceStatus?: ReportOutcome | null;
  /** Worst fraud-risk level among the case's documents, if any were analysed. */
  fraudStatus?: string | null;
}

/**
 * One page of the CAF instructor list — `GET /agent/cases/list`.
 *
 * `total` is the filtered row count across every page, not this page's
 * length: the client needs it to render pagination without a second request.
 */
...
export interface CaseCitizen {
  ...
  lastName: string;
  email: string;
  /** Absent when the citizen has not declared a birth date. */
...
export interface CaseDecision {
  ...
  decidedBy: string;
  /**
   * Outcome of the automatic decision e-mail: `"sent"`, `"failed"`, or `null`
   * when there was no linked citizen account to notify. The decision is saved
   * regardless of this value — a failed send is reported, never an error.
   */
