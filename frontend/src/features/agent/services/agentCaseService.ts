import type { Case, CaseListPage, CaseQueueStats, CaseSortField, CaseSummary, SortDirection } from '@/types';
...
export interface AgentCaseService {
  ...
  /**
   * CAF instructor list — paginated, sortable, filterable.
   * Backend endpoint: `GET /agent/cases/list`.
   */
  listCasePage(query?: CaseListQuery): Promise<CaseListPage>;
...
/** Server-side pagination, sorting and filtering for the CAF instructor list. */
export interface CaseListQuery {
  status?: string;
  /** Free-text search across the application reference and the citizen's name. */
  search?: string;
...
export const httpAgentCaseService: AgentCaseService = {
  ...
  listCasePage: (query = {}) =>
    apiClient.get<CaseListPage>('/agent/cases/list', {
      params: {
        status: query.status,
        search: query.search,
        sortBy: query.sortBy,
