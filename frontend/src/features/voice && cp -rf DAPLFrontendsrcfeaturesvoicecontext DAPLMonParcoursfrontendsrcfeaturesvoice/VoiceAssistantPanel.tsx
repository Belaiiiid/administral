...
import { Home, Files, Volume2, Square, HelpCircle, AlertCircle, Mic, MicOff } from 'lucide-react';
...
export const VoiceAssistantPanel: React.FC = () => {
  ...
  const handleReadPage = () => {
    if (pageContext?.readableText) {
      speakText(pageContext.readableText);
    } else {
      speakText("Il n'y a aucun contenu lisible sur cette page.");
    }
  };

  const handleExplainActions = () => {
    if (pageContext?.actions && pageContext.actions.length > 0) {
      const descriptions = pageContext.actions.map((a) => a.description).join('. ');
      speakText(`Sur cette page, tu peux : ${descriptions}`);
    } else {
      speakText("Aucune action n'est disponible sur cette page.");
    }
  };

  // Status-specific labels & colors
  ...
  if (status === 'standby') {
    statusText = "En veille — cliquez sur le micro pour parler";
    statusColorClass = 'bg-outline text-on-surface-variant';
  } else if (status === 'listening') {
    statusText = isWaitingForConfirmation ? 'Confirme par "oui" ou "non"' : 'Je t\'écoute…';
    statusColorClass = 'bg-primary text-white';
    statusAnimationClass = 'animate-pulse';
  } else if (status === 'speaking') {
    statusText = 'Parle…';
    statusText = 'Parle...';
    statusColorClass = 'bg-secondary text-white';
  } else if (status === 'error') {
    statusText = 'Erreur';
    statusColorClass = 'bg-destructive text-white';
  }
    ...
    <div className="fixed bottom-4 right-4 z-50 flex w-full max-w-sm flex-col rounded-2xl border border-border bg-surface-lowest/90 p-5 shadow-2xl backdrop-blur-md transition-all duration-300 md:bottom-8 md:right-8 animate-in fade-in slide-in-from-bottom-5">
      ...
      <div className="my-4 flex flex-col gap-3 rounded-xl bg-surface-low p-4 min-h-24 justify-center">
        ...
        {status === 'error' && error ? (
          ...
          <div className="flex gap-2 items-start">
            <Mic className="size-4 shrink-0 mt-1 text-primary animate-pulse" />
            <p className="text-body-md text-on-surface italic">"{transcript}"</p>
          </div>
        ) : (
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            variant="outline"
            className="flex items-center justify-start gap-2 h-11 text-body-sm rounded-xl"
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          <Button
            variant="default"
            className="flex items-center justify-center gap-2 h-11 rounded-xl select-none"
