...
interface VoiceAssistantContextType {
  ...
  stopSpeaking: () => void;
  startListening: () => void; // one-shot capture
  startPushToTalk: () => void; // begin PTT capture
  stopPushToTalk: () => void;  // end PTT capture and process
}
...
export const VoiceAssistantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Page context to get active fields/actions
  ...
  // Instantiations inside refs to maintain identity across renders
  ...
  // Standby/listening window management
  ...
  function clearListenTimeout() {
    if (listenTimeout.current) {
      window.clearTimeout(listenTimeout.current);
      listenTimeout.current = null;
    }
  }

  function startStandby() {
    // Manual activation only: stop any STT recorder and sit idle in standby
    sttProvider.current?.stop();
    sttProvider.current = null;
    setStatus('standby');
  }

  function startListeningWindow(sliceMs = 6000) {
    // One-shot capture; first transcript becomes the command
    sttProvider.current?.stop();
    commandWindowActive.current = true;
    sttProvider.current = new MistralSttProvider({ sliceMs });
    sttProvider.current.onError((errMsg) => {
      setError(errMsg);
      setStatus('error');
    });
    sttProvider.current.onTranscript((text) => {
      if (!commandWindowActive.current) return;
      commandWindowActive.current = false;
      clearListenTimeout();
      // Forward as a command
      triggerCommand(text);
      // Stop STT after one command
      sttProvider.current?.stop();
      setStatus('standby');
    });
    setStatus('listening');
    sttProvider.current.start();
    clearListenTimeout();
    listenTimeout.current = window.setTimeout(() => {
      // Timeout without command → back to standby
      commandWindowActive.current = false;
      sttProvider.current?.stop();
      startStandby();
    }, sliceMs);
  ...
  useEffect(() => {
    ...
    // Wire TTS onEnd → always return to standby in voice mode
    ttsProvider.current.onEnd(() => {
      if (useVoiceStore.getState().modeVocal) {
        startStandby();
      } else {
        setStatus('idle');
      }
  ...
  const triggerCommand = useMemo(() => {
    return (text: string) => {
        ...
        (async () => {
          try {
            ...
            if (resp.ok) {
              ...
              // Map classifier output to deterministic intents or reply
              ...
              if (data.type === 'reply' && data.reply) {
                if (ttsProvider.current) {
                  sttProvider.current?.stop();
                  setStatus('speaking');
                  ttsProvider.current.speak(String(data.reply));
                }
                return;
              }
              if (data.type === 'fill_field' && data.value) {
                ...
                if (activeFieldsList.length === 1) {
                  targetFieldId = activeFieldsList[0].fieldId;
                } else {
          ...
          if (ttsProvider.current) {
            sttProvider.current?.stop();
            setStatus('speaking');
            ttsProvider.current.speak("Désolé, je n'ai pas compris la commande.");
          }
        })();
        return;
      }
      ...
      // Stop listening while executor is working / speaking
      sttProvider.current?.stop();
      setStatus('speaking');

      executor.current.execute(intent, deps);

      // If the intent didn't cause speech synthesis to start, go back to standby
      setTimeout(() => {
  ...
  // Wire STT onTranscript
  // Handle modeVocal changes
  useEffect(() => {
    if (modeVocal) {
      setError(null);
      startStandby();
  ...
  const speakText = (text: string) => {
    if (ttsProvider.current) {
      sttProvider.current?.stop();
      setStatus('speaking');
      ttsProvider.current.speak(text);
    }
  };
  ...
  const stopSpeaking = () => {
    if (ttsProvider.current) {
      ttsProvider.current.stop();
      if (modeVocal) {
        // In push-to-talk mode, do not auto-reopen the mic; return to standby
        startStandby();
      } else {
        setStatus('idle');
      }
  ...
  // Push-to-talk: start with long slices while pressed, stop and process on release
  ...
  const startPushToTalk = () => {
    ...
    setTranscript("");
    // Use short slices so chunks flush while holding the button
    sttProvider.current?.stop();
    commandWindowActive.current = true;
    const provider = new MistralSttProvider({ sliceMs: 1500, deferUpload: true });
    sttProvider.current = provider;
    provider.onError((errMsg) => {
      setError(errMsg);
      setStatus('error');
    });
    provider.onTranscript((text) => {
      // With deferUpload, transcript arrives once after stop(); accept it and act
      setTranscript(text);
      setStatus('listening');
      // Execute immediately on final transcript
      triggerCommand(text);
      sttProvider.current?.stop();
      setStatus('standby');
    });
  ...
  const stopPushToTalk = () => {
    // In deferred-upload mode, final transcript arrives via onTranscript after stop()
    commandWindowActive.current = false;
    sttProvider.current?.stop();
    setStatus('standby');
  };
