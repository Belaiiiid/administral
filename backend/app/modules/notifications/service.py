...
def emit_decision(
    ...
    *,
    ...
    try:
        ...
        # An e-mail copy, only if the citizen opted in (default on). Best-effort
        # and after the in-app notification is safely committed: the email
        # backend is a swappable interface (console in dev, SMTP in prod), so a
        # send failure here must not undo the notification the citizen can still
        # see in the app.
        if settings_service.wants_email(db, citizen_user_id):
            recipient = auth_repository.get_by_id(db, citizen_user_id)
            if recipient is not None:
